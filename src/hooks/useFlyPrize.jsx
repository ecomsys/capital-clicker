...
export const useFlyPrize = () => {
  ...
  const createFlyingCoinToTarget = useCallback((startX, startY, targetRect) => {
    return new Promise((resolve) => {
      if (!targetRect || targetRect.width === 0 || targetRect.height === 0) {
        resolve(); // ничего не летит – сразу завершаем
        return;
      }
      if (activeAnimations.current.size >= 5) {
        resolve(); // лимит – не создаём анимацию
        return;
      }
      ...
      animation.onfinish = () => {
        ...
        resolve(); // анимация завершена – разрешаем Promise
      };
    });
  }, []);
  ...
  const flyPrizeFromPoint = useCallback((startX, startY, targetSelector = ".user-balance") => {
    ...
    if (targetRect.width === 0) return Promise.resolve();
    return createFlyingCoinToTarget(startX, startY, targetRect);
  ...
  const flyPrizeFromElement = useCallback((element, targetSelector = ".user-balance") => {
    ...
    return flyPrizeFromPoint(startX, startY, targetSelector);
  }, [flyPrizeFromPoint]);
